...
import {
  getMachineKnottingCountCount,
  getMachineWeavingData,
} from "../../components/functions/production/weavingProductionFunctions";

export default function useWeavingData(selectedWeek) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false); // <- boolean for easy UI checks
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!weavingRows || weavingRows.length === 0) {
          if (mounted) {
            setError(true); 
            setLoading(false);
          }
